...
let chatClient = null;
// ============ 工具 1: 获取竞技场状态 ============
server.tool("get_arena_status", // 工具名称
"获取大逃杀房间的实时上下文：房间状态、所有玩家及其人性分、最近聊天记录、当前轮次投票和淘汰历史。在采取行动前使用此工具了解完整的游戏情况。", {
    ...
    try {
        ...
        const fromBlock = Math.max(roomInfo.startBlock > 0 ? Number(roomInfo.startBlock) : 0, currentBlock - 100);
        // 分页查询事件（每次最多 100 个区块）
        const queryEventsInBatches = async (filter, start, end, filterName) => {
            const allEvents = [];
            ...
            while (currentFrom <= end) {
                ...
                try {
                    ...
                    allEvents.push(...batch.filter((e) => "args" in e));
                    if (batchCount % 10 === 0) {
        ...
        const [playerInfos, playerNames] = await Promise.all([
            Promise.all(playerAddresses.map(async (addr) => {
                await rpcRateLimiter.wait();
        ...
        // 格式化玩家信息，提取关键字段
        const formattedPlayers = playerInfos.map((p, idx) => ({
            address: p.addr, // 玩家地址
            name: names[idx] || "", // 玩家名称
...
// ============ 工具 2: 执行链上操作 ============
server.tool("action_onchain", // 工具名称
"执行链上操作：CHAT（发送消息）或 VOTE（投票淘汰）。", {
    ...
    if (!playerWallet) {
        ...
        return {
            content: [{ type: "text", text: "Error: Wallet not initialized. Use init_session first." }],
            isError: true,
        };
    }
    try {
            ...
            try {
                ...
                if (identityRes.ok) {
                    ...
                    if (!identity.isAI) {
                        ...
                        return {
                            content: [{ type: "text", text: "Error: You joined this room as a Human (via browser). MCP actions are disabled — use the web UI to play." }],
                            isError: true,
                        };
                    }
                }
        ...
        switch (type) {
            case "CHAT": // 聊天操作 — 走链下 REST API
                if (!content)
                    throw new Error("Content required for CHAT action");
                if (content.length > 280)
                    throw new Error("Message too long (max 280 chars)");
                if (!chatClient)
                    throw new Error("Chat client not initialized. Use init_session first.");
            ...
            case "VOTE": // 投票操作
                if (!target)
                    throw new Error("Target address required for VOTE action");
...
// ============ 工具 3: 检查会话状态 ============
server.tool("check_session_status", // 工具名称
"检查当前钱包的地址和 PAS 余额。在采取行动前使用此工具验证会话是否处于活跃状态。", {}, // 无需参数
async () => {
    // 检查钱包是否已初始化
    if (!playerWallet) {
        return {
            content: [{ type: "text", text: "Session wallet not initialized. Use init_session first." }],
            isError: true,
        };
    }
    try {
...
"通过结算当前轮次推进游戏。任何人都可以在经过足够的区块后调用此函数。触发淘汰得票最多的玩家。", {
    ...
    if (!playerWallet) {
        return {
            content: [{ type: "text", text: "Error: Session not initialized. Use init_session first." }],
            isError: true,
        };
    }
    try {
...
"游戏结束后领取你的 PAS 奖励。返回奖励金额和交易哈希。", {
    ...
    try {
        ...
        if (claimed) {
            return {
                content: [{ type: "text", text: "Reward already claimed for this room." }],
            };
        }
        // 检查奖励金额是否为零
        if (amount === 0n) {
            return {
                content: [{ type: "text", text: "No reward available for this room." }],
            };
        }
...
// ============ 工具 8: 获取轮次状态 ============
server.tool("get_round_status", // 工具名称
...
"创建一个新的游戏房间。你成为房间创建者并自动加入（收取入场费）。Tier 控制游戏节奏：Quick (0) = 快速轮次，Standard (1) = 平衡，Epic (2) = 长游戏。返回新房间 ID。", {
    ...
    try {
        ...
        if (activeRoom > 0n) {
            return {
                content: [{ type: "text", text: `已在房间 #${activeRoom} 中。请先离开或完成游戏再创建新房间。` }],
                isError: true,
            };
        }
        ...
        if (!chatClient) {
            return {
                content: [{ type: "text", text: "Error: Chat client not initialized. Use init_session first." }],
                isError: true,
            };
        }
...
// ============ 工具 15: 获取游戏历史 ============
server.tool("get_game_history", // 工具名称
"获取完整的游戏历史：每轮的所有投票、淘汰顺序和游戏结果。最适合在游戏结束后使用或回顾过去的游戏。", {
    ...
    try {
        ...
        for (const e of voteEvents) {
            ...
            if (!rounds[r])
                rounds[r] = { votes: [], eliminated: null };
            rounds[r].votes.push({
                voter: ev.args[1], // 投票者
        ...
        for (const e of elimEvents) {
            ...
            if (!rounds[elimRound])
                rounds[elimRound] = { votes: [], eliminated: null };
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        eliminationOrder: eliminationOrder, // 淘汰顺序
                        gameStats, // 游戏统计信息
                    }, null, 2),
                },
            ],
...
"通过扫描可用房间并加入第一个匹配项来进行匹配进入等待中的房间。检查 AI 插槽可用性（MCP = AI）。如果没有房间匹配，建议使用 create_room。", {
    ...
    try {
        ...
        if (activeRoom > 0n) {
            return {
                content: [{ type: "text", text: `已在房间 #${activeRoom} 中。请先离开或完成游戏再加入其他房间。` }],
                isError: true,
            };
        }
        // 获取房间总数
        ...
        if (total === 0) {
            return {
                content: [{ type: "text", text: "No rooms exist yet. Use create_room to create one." }],
            };
        }
        // 设置过滤条件（使用提供的值或默认值）
        ...
        for (let i = total; i >= 1; i--) {
            ...
            // 阶段必须是等待（0）且未满员
            if (phase !== 0 || playerCount >= maxP)
                continue;
            // 房间大小过滤器
            if (maxP < filterMinPlayers || maxP > filterMaxPlayers)
                continue;
            // 费用过滤器
            if (entryFee < feeMinWei || entryFee > feeMaxWei)
                continue;
            // 等级过滤器（如果指定）
            if (tier !== undefined && roomTier !== Number(tier))
                continue;
            ...
            if (!chatClient) {
                return {
                    content: [{ type: "text", text: "Error: Chat server not configured (CHAT_SERVER_URL required for commit-reveal join)." }],
                    isError: true,
                };
            }
